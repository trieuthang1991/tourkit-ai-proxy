...
// Empty form mặc định. Bỏ `services` (Dịch vụ điều hành chi) — không còn dùng.
const EMPTY = () => ({
  title: '', marketName: '', tourType: '',
  startDate: '', endDate: '', adultCount: 0, childCount: 0,
  customerName: '', customerPhone: '', customerEmail: '',
  note: '', expenses: [], warnings: [],
});

const SAMPLE = `Tour Đà Nẵng - Hội An 3N2Đ, đi 15/7 về 17/7, đoàn 18 người lớn + 4 trẻ em, KH chị Nguyễn Thị Lan SĐT 0987 123 456, email lan@gmail.com. Vé tour 5,5tr/người lớn, trẻ em 3,5tr (VAT 8%). Phụ thu phòng đơn 800k/người. Bảo hiểm 50k/người. Yêu cầu: 1 phòng cho người ăn chay.`;

// Fallback markets nếu chưa đăng nhập / endpoint lỗi.
const MARKETS_FALLBACK = ['Nội địa', 'Inbound', 'Outbound', 'Châu Á', 'Châu Âu', 'Châu Mỹ', 'Châu Úc', 'Hàn Quốc', 'Nhật Bản', 'Thái Lan', 'Đài Loan', 'Trung Quốc'];

function MarketSelect({ value, onChange }) {
  const [markets, setMarkets] = _tS(MARKETS_FALLBACK);
  _tE(() => {
    let alive = true;
    (async () => {
      try {
        const r = await window.tourkitAuth.authedFetch('/api/v1/markets');
        if (!r.ok) return;   // 401/502 → giữ fallback
        const list = await r.json();
        if (alive && Array.isArray(list) && list.length > 0) setMarkets(list);
      } catch { /* giữ fallback */ }
    })();
    return () => { alive = false; };
  }, []);
...
function TourBuilderPage({ pushToast }) {
  ...
  const [draftStatus, setDraftStatus] = _tS('pristine');
  const [lastDraftAt, setLastDraftAt] = _tS(null);    // ISO timestamp Redis draft
  const [lastCommitAt, setLastCommitAt] = _tS(null);  // ISO timestamp SQL commit
  const autosaveTimer = _tR(null);
  const skipNextAutosave = _tR(false);                // sau khi load OR commit → bỏ qua 1 lần autosave
  ...
  _tE(() => {
    ...
    (async () => {
      try {
        ...
        // Endpoint mới trả {item, draftSavedAt, isDraft} — backward compat với raw item
        const item = j.item || j;
        const data = item.data || j.data;
        if (data) setForm({ ...EMPTY(), ...data, expenses: data.expenses || [] });
        setSavedId(item.id || j.id);
        if (j.draftSavedAt) {
          setDraftStatus('draft');
          setLastDraftAt(j.draftSavedAt);
        } else {
          setDraftStatus('committed');
          setLastCommitAt(item.updatedAt);
        }
        skipNextAutosave.current = true;   // load xong tránh tự autosave ngay
        pushToast('Đã mở báo giá "' + (item.title || item.id || j.id) + '"' + (j.isDraft ? ' (đang nháp)' : ''));
  ...
  async function aiFill() {
    ...
    try {
      ...
      const r = await window.tourkitAuth.authedFetch('/api/v1/tour-builder/parse', {
        ...
        body: JSON.stringify({ prompt, provider: cfg.provider, model: cfg.model }),
      });
  ...
  // ── Pricing engine — live recalc (REVENUE-ONLY sau khi bỏ Dịch vụ điều hành).
  // Cost side đã chuyển sang Wizard báo giá riêng. Tour-builder chỉ bóc Phần thu.
  // Quy đổi pax: trẻ em = 0.75 người lớn.
  ...
  // Build body chung cho draft + commit
  const buildSaveBody = () => ({
    id: savedId,
  ...
  // ── AUTOSAVE → Redis (POST /draft) — KHÔNG đụng SQL, debounce 1.5s sau keystroke cuối.
  // Skip nếu form rỗng / vừa load / vừa commit. Update savedId nếu server sinh GUID mới.
  async function autosaveDraft() {
    if (!form.title && !form.customerName && form.expenses.length === 0) return;
    ...
    try {
      ...
      console.warn('[autosave] fail:', e.message);
    }
  }

  // Watch form change → debounce 1.5s → autosave to Redis
  _tE(() => {
    if (skipNextAutosave.current) { skipNextAutosave.current = false; return; }
    setDraftStatus(prev => prev === 'committed' ? 'dirty' : prev === 'pristine' ? 'dirty' : prev);
  ...
  // Save báo giá → POST /api/v1/tour-quotes (COMMIT vào SQL — explicit user action).
  async function saveQuote() {
    ...
    setSaving(true);
    try {
      // Cancel pending autosave (sẽ commit luôn data hiện tại)
  ...
  const hasForm = form.title || form.expenses.length > 0;
  return (
    <main className="page tb">
      <window.PageShell.PageHero
        ...
        status={{ label: hasForm ? 'CÓ BẢN NHÁP' : 'CHƯA CÓ NHÁP',
          detail: hasForm ? `${form.expenses.length} khoản thu` : 'Nhập mô tả & bấm AI',
          tone: hasForm ? 'live' : 'idle' }}
        actions={<>
          {/* Draft/Commit status chip — báo user biết data đang ở Redis hay đã commit DB */}
          {savedId && draftStatus !== 'pristine' && (() => {
            const fmt = (iso) => {
              if (!iso) return '';
            ...
            const map = {
              dirty:      { txt: '● Đã sửa, chờ autosave…', cls: 'tb-chip-warn' },
              autosaving: { txt: '↻ Đang autosave Redis…', cls: 'tb-chip-info' },
              draft:      { txt: '◐ Đang nháp (Redis · ' + fmt(lastDraftAt) + ')', cls: 'tb-chip-warn' },
              committed:  { txt: '✓ Đã commit DB · ' + fmt(lastCommitAt), cls: 'tb-chip-ok' },
            };
            const m = map[draftStatus];
            if (!m) return null;
            return <span className={'tb-chip ' + m.cls}
              title="Edit form → autosave Redis 1.5s sau · Bấm Commit để ghi SQL">{m.txt}</span>;
          })()}
      ...
      <div className="tb-grid">
        ...
        <section className="tb-right">
          ...
          {/* Tổng kết — REVENUE-ONLY sau khi bỏ Dịch vụ điều hành.
              Cost / margin chuyển sang Wizard báo giá riêng. */}
          <div className="tb-summary">
            <div><span>Tổng thu (sau VAT)</span><b className="good">{vnd(Math.round(sumExp))} ₫</b></div>
            <div><span>Thu / pax quy đổi</span><b>{sumExp > 0 ? vnd(Math.round(revPerPax)) + ' ₫' : '—'}</b></div>
            <div>
              <span>Pax quy đổi</span>
              <b style={{color: 'var(--text-2)'}}>
                {paxEquiv.toFixed(2)} <small style={{fontSize:10,opacity:.7}}>(trẻ × 0.75)</small>
              </b>
            </div>
            <div>
              <span>Bước tiếp theo</span>
              <b style={{color: 'var(--text-2)', fontSize: 12, fontWeight: 500}}>
                Tính giá vốn ở Wizard
              </b>
            </div>
          </div>
        </section>
