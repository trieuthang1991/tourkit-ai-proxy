  ...
  window.tourkitFlows.register({
    ...
    nodes: [
      ...
      F.node('e4', 'step', F.LEFT, 3.2, { icon: 'dollar', title: 'Tài chính 2 kỳ',
        sub: 'Đầu tháng tới nay so CÙNG KHOẢNG NGÀY của kỳ đối chiếu', cfg: ['comparePeriod'] }),
      F.node('e5', 'step', F.MID, 3.2, { icon: 'calendar', title: 'Lịch hẹn hôm nay',
        sub: 'Kèm số tồn đọng quá hạn tích luỹ', cfg: ['secAppointments'] }),
      F.node('e6', 'step', F.RIGHT, 3.2, { icon: 'users', title: 'Top nhân viên bán hàng',
        sub: 'Doanh số từ đầu tháng', cfg: ['secSellers', 'sellerCount'] }),
      ...
      F.node('e9b', 'step', F.MID, 6.1, { icon: 'trend', title: 'Ước cả tháng có kịp kế hoạch?',
        sub: 'Chưa khai chỉ tiêu thì bỏ qua · 4 ngày đầu tháng chỉ báo số thực đạt, không ước',
        cfg: ['secForecast', 'revenueTarget'] }),
      F.node('e10', 'branch', F.MID, 6.9, { icon: 'sparkle', title: 'AI viết nhận định được không?',
        sub: 'Ai thấy CÙNG bộ số thì dùng chung 1 bản → thường 1 lượt/công ty', cfg: ['useAi'] }),
      F.node('e11', 'step', F.LEFT, 8.1, { icon: 'edit', title: '5–8 câu nhận định',
        sub: 'Cấm bịa số ngoài bảng · tồn đọng không được gọi là việc khẩn hôm nay',
        cfg: ['showNumbers'] }),
      F.node('e12', 'step', F.RIGHT, 8.1, { icon: 'checkCircle', title: 'Bảng số thuần',
        sub: 'AI lỗi hoặc hết lượt → bản tin KHÔNG bao giờ mất' }),

      // Hai TẦNG cấu hình (giống bản tin của nhân viên bán hàng)
      F.node('e13', 'step', F.LEFT, 9.3, { icon: 'shield', title: 'OA Zalo của công ty',
        sub: 'Tốn tiền + hạn mức riêng nên công ty tự khai · Telegram và email hệ thống lo' }),
      F.node('e14', 'step', F.RIGHT, 9.3, { icon: 'user', title: 'Nơi nhận của bạn',
        sub: 'Email · Zalo · Telegram — tự khai, muốn tắt kênh nào cũng được' }),

      F.node('e15', 'step', F.MID, 10.5, { icon: 'bell', title: 'Lưu vào Bảng tin (luôn luôn)',
        sub: 'Kho lưu để xem/nghe lại · bảng số luôn đính kèm dưới bài viết để đối chiếu' }),
      F.node('e16', 'send', F.MID, 11.7, { icon: 'send', title: 'Xếp hàng đợi — đến giờ mới gửi',
