            ...
            {day.activities.map((a, ai) => (
              <div key={a.id} className="activity" onClick={() => setEditing({dayIdx: activeDay, actIdx: ai})}>
                ...
                <div className="activity-cost">
                  ...
                  {/* Lộ cách ước lượng: tổng ≈ đơn giá/khách × số khách — để user biết AI ước theo đầu người. */}
                  {totalPax > 0 && (a.cost || 0) > 0 && (() => {
                    const ct = a.costType || ({HOTEL:'pax',MEAL:'pax',TICKET:'pax'}[a.type] || 'shared');
                    const perPax = Math.round((a.cost || 0) / totalPax);
                    return (
                      <div className="activity-cost-perpax" style={{fontSize: 10.5, color: 'var(--text-3)', marginTop: 3, whiteSpace: 'nowrap'}}>
                        {ct === 'pax'
                          ? <>≈ <strong style={{color: 'var(--text-2)'}}>{fmtVND(perPax)}</strong>/khách × {totalPax} khách</>
                          : <>chia {totalPax} khách ≈ <strong style={{color: 'var(--text-2)'}}>{fmtVND(perPax)}</strong>/khách</>}
                      </div>
                    );
                  })()}
                </div>
          ...
          <window.HotelPickerModal
            ...
            onPick={(data, scope) => {
              ...
              const applyPack = (a) => ({
                ...
                cost:        data.cost,
                supplierDesc: data.supplierDesc || '',   // = provider_service_pricing.description phòng đã chọn → fill ô "Mô tả NCC"; KHÔNG đè a.description (lịch trình)
...
async function fetchNccServices(provId, catId) {
  try {
    ...
    const data = await r.json();
    const arr = Array.isArray(data) ? data : (data.items || data.Items || []);
    return arr.map(s => ({ id: s.id ?? s.Id, name: s.name ?? s.Name, contractPrice: s.contractPrice ?? s.ContractPrice ?? 0, publicPrice: s.publicPrice ?? s.PublicPrice ?? 0, description: s.description ?? s.Description ?? '' }));
  } catch { return []; }
}
...
function EditServiceModal({ data, dayNum, onClose, onSave, onDelete, totalPax }) {
  ...
  const applyService = (p, svc) => setForm(f => ({ ...f,
    ...
    supplierDesc: (svc.description || f.supplierDesc || ''),        // = provider_service_pricing.description → fill ô "Mô tả NCC" (lấy y như giá Net)
    ...
    <div className="modal-backdrop">
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div className="modal-left" style={{maxHeight: '88vh', minHeight: 0}}>
          ...
          <div className="field">
            <label className="label">Mô tả Dịch vụ / chi tiết phòng</label>
            <input className="input" value={form.supplierDesc || ''} placeholder="Mô tả dịch vụ"
              onChange={e => setForm(f => ({...f, supplierDesc: e.target.value}))} />
          </div>
