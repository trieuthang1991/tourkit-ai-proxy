...
public interface IMailSender
{
    ...
    Task SendAsync(string toEmail, string? toName, string subject, string body, string? inReplyToMessageId, CancellationToken ct);

    /// Tiện ích trả lời 1 email gốc (giữ subject "Re:" + gắn In-Reply-To).
    Task SendReplyAsync(MailItem original, string body, CancellationToken ct)
    {
        ...
        return SendAsync(original.From.Email, original.From.Name, reSubj, body, mid, ct);
    }
}
