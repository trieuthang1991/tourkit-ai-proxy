...
public class NativeToolUseAgent : IAgentRuntime
{
    ...
    private static readonly string SystemPromptBase =
        ...
        "KHI USER YÊU CẦU LÀM một việc (không phải hỏi số liệu) — vd 'giao việc cho NV X', 'trả lời khách Y', 'soạn mail mới', 'đánh giá/xếp hạng khách hàng Z', 'chấm điểm deal/cơ hội', 'kiểm tra mail mới', 'đặt lịch hẹn với khách', 'sắp gặp/sắp gọi khách W, chuẩn bị giúp' — BẮT BUỘC gọi ĐÚNG action tool tương ứng (check_mail/send_mail_reply/compose_mail/review_customer/prepare_meeting/score_deal/assign_task/create_appointment), KHÔNG gọi tool đọc số liệu (vd top_customers) để thay thế. Điền params từ câu nói + ngữ cảnh lượt trước (vd 'khách này' → tên đã nhắc), KHÔNG tự bịa id. " +
