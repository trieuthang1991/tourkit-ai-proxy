...
| 9 | `dbo.WidgetTokens` | Chat widget tokens — tenant gen token paste vào `<script data-token="">` ở site khách. Đã wire qua CRM TourKit — `TourKitSessionId` + `AllowedTools` (xem [`WidgetChatCrmService`](../Services/Widget/WidgetChatCrmService.cs)). | [`WidgetTokenRepository`](../Services/Widget/WidgetTokenRepository.cs) | `Token` |
