...
| 18 | `dbo.OutboundMails` | **Hàng đợi gửi ra ngoài đa kênh dùng chung** (producer enqueue `Status=0`; worker riêng — CEO viết — render template + gửi). Nội dung theo TEMPLATE: `TemplateCode` + `[Params]` JSON (không soạn HTML ở proxy). `Status` TINYINT (0=pending..4=skipped) cho worker dùng enum. **`Channel` TINYINT (0=email 1=telegram 2=zalo, default 0 — enum [`OutboundChannel`](../Services/Digest/OutboundChannel.cs))**: dòng cũ trước khi có cột này tự mang giá trị 0 (email), không cần migrate data. Khác `dbo.Mails` (inbox). Producer đầu tiên: workflow `deal-auto-review` (Kind=`deal-cooling-alert`). | [`MailQueueRepository`](../Services/Mail/MailQueueRepository.cs) | `Id` IDENTITY |
